...
def step(gamma):
	result = subprocess.run(["bgmc","-c","-N",str(N),"-g",str(gamma),"-n","1","-e",str(extraModePairs),*options],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	...
	if cmp[0]>0.0 and cmp[1]>0.0:
		...
		# shouldn't happen
		exit(2)
	print(str(gamma).rjust(16),order,*cmp,lines,flush=True)
	return order
